...
def nash_sutcliffe_efficiency(
    observed: Sequence[float], simulated: Sequence[float]
) -> float:
    """
    ...
    if len(observed) == 0:
        raise ValueError("Length of observed data must be greater than zero.")
    ...
    if denominator == 0:
        raise ValueError(
            "Denominator in NSE calculation is zero; check observed values."
        )
