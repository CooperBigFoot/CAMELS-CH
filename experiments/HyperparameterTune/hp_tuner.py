...
class HyperparameterTuner:
    ...
    def save_study_results(self):
        ...
        # Save to CSV
        results_df.to_csv(
            self.dirs['results'] / f"{self.model_type}_optimization_results.csv", index=False
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hyperparameter tuning for hydrological models")
    parser.add_argument("--model", type=str, required=True, choices=["tide", "tsmixer", "ealstm", "tft"],
    ...
    study_name = args.study_name or f"{args.model}_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        if study and study.trials:
            from experiments.HyperparameterTune.utils import generate_optimization_report
            generate_optimization_report(
                study,
                args.model,
                tuner.dirs['visualizations']
            )
    finally:
        tuner.cleanup()
