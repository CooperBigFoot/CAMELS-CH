...
pretrain_trainer.fit(pretrain_model, ch_data_module)

# Save pretrained model
pretrain_save_path = Path(
    "experiments/Exp3/saved_models/tsmixer_challenger_pretrained.pt"
)
pretrain_save_path.parent.mkdir(parents=True, exist_ok=True)
torch.save(pretrain_model.state_dict(), pretrain_save_path)
...
# Initialize new model with lower learning rate for fine-tuning
print("CONFIGURING MODEL FOR FINE-TUNING")
finetune_model = LitTSMixer(
    input_len=config.INPUT_LENGTH,
    output_len=config.OUTPUT_LENGTH,
    input_size=len(ts_columns),
    static_size=len(static_columns) - 1,
...
pretrained_state = torch.load(pretrain_save_path)
...
finetune_trainer.fit(finetune_model, ca_data_module)

# ----- SAVE FINAL MODEL -----
print("SAVING FINAL MODEL")
model_save_path = Path("experiments/Exp3/saved_models/tsmixer_challenger.pt")
model_save_path.parent.mkdir(parents=True, exist_ok=True)
torch.save(finetune_model.state_dict(), model_save_path)
...
finetune_trainer.test(finetune_model, ca_data_module)
raw_results = finetune_model.test_results

# Create evaluator and calculate metrics
print("Calculating evaluation metrics...")
evaluator = TSForecastEvaluator(
    ca_data_module, horizons=list(range(1, finetune_model.config.pred_len + 1))
)
