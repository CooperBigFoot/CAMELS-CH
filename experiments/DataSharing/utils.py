...
def setup_dirs(output_dir: str) -> Dict[str, Path]:
    ...
    for country in countries:
        for model_type in model_types:
            (dirs["checkpoints"] / country / model_type).mkdir(parents=True, exist_ok=True)
...
def load_country_data(
    ...
    if country and country.lower() != 'combined':
        ...
        country_gauge_ids = all_static_data[
            all_static_data["country"] == country
        ][config.GROUP_IDENTIFIER].unique()
        
        if len(country_gauge_ids) == 0:
            raise ValueError(f"No basins found for country: {country}")
        
        # Filter time series and static data
        ts_data = all_ts_data[all_ts_data[config.GROUP_IDENTIFIER].isin(country_gauge_ids)]
        static_data = all_static_data[all_static_data[config.GROUP_IDENTIFIER].isin(country_gauge_ids)]
...
def load_model_configs_and_datamodules(
    ...
    for model_type, yaml_path in yaml_paths.items():
        ...
        try:
            ...
            print(f"Created DataModule for {model_type} with input_length={input_length}, "
    ...
    for run in range(num_runs):
        ...
        try:
            ...
            run_results = {
                ...
                "checkpoint_path": str(checkpoint_dir / f"run_{run}" / 
            ...
            print(f"Run {run+1} completed with best val_loss: {best_val_loss:.4f} at epoch {trainer.current_epoch}")
...
def save_experiment_results(results: Dict[str, Any], output_dir: str, country: str) -> None:
    """
