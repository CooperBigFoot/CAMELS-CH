...
def map_clusters_to_basins(
    ...
    cluster_map = dict(
        zip(cluster_assignments["gauge_id"], cluster_assignments["cluster"])
    )
    ...
    if missing_clusters > 0:
        ...
        print(
            f"Example missing basins: {static_df[static_df['cluster'].isna()]['gauge_id'].head().tolist()}"
        )
...
def get_labelled_data_by_country(
    ...
    missing_cols = [
        col for col in config.statics_to_keep if col not in static_df.columns
    ]
    if missing_cols:
    ...
    print(
        f"  Processed {len(result_df)} basins with valid data and cluster assignments for {country}"
    )
...
def create_mutual_info_heatmap(X: pd.DataFrame, output_path: str) -> None:
    ...
    sns.heatmap(
        mi_matrix.astype(float), annot=True, fmt=".2f", cmap="coolwarm", linewidths=0.5
    )
...
def train_rf_classifier(
    ...
    if X_numeric.isna().any().any():
        print(
            "Warning: Missing values detected in features, filling with median values"
        )
    ...
    try:
        ...
        proba_predictions = cross_val_predict(
            best_rf, X_numeric, y, cv=cv, method="predict_proba"
        )
...
def plot_feature_importance(
    model: RandomForestClassifier, feature_names: List[str], output_path: str
) -> None:
    """
...
def predict_target_clusters(
    ...
    if X_numeric.isna().any().any():
        print(
            "Warning: Missing values detected in target features, filling with median values"
        )
        X_numeric = X_numeric.fillna(X_numeric.median())
    ...
    try:
        ...
        proba_df = pd.DataFrame(
            proba_predictions,
            columns=[
                f"cluster_{model.classes_[i]}_prob" for i in range(len(model.classes_))
            ],
        )
...
def main(config: ClassificationConfig) -> None:
    ...
    feature_columns = [
        col for col in combined_data.columns if col not in ["gauge_id", "cluster"]
    ]
    ...
    target_feature_columns = [
        col for col in feature_columns if col in target_data.columns
    ]
