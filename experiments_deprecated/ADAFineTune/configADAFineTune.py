...
class ExperimentConfig:
    """Configuration for adversarial fine-tuning domain adaptation experiments."""

    EXPERIMENT_NAME: str = "adversarial_finetune"
    # Base configuration
    ...
    VIZ_MAX_SAMPLES: int = 800  # Maximum samples per domain for visualization
    VIZ_PERPLEXITY: int = 30  # t-SNE perplexity parameter
    VIZ_DPI: int = 300  # DPI for saving visualizations

    # Fine-tuning options
    FREEZE_BACKBONE: bool = True  # Whether to freeze backbone during fine-tuning
    # FREEZE_LAYERS: int = (
    #     3  # Number of layers to freeze (if not freezing entire backbone)
    # )
    ...
    def _validate_config(self):
        ...
        if self.VIZ_MAX_SAMPLES <= 0:
            raise ValueError("VIZ_MAX_SAMPLES must be positive")
    ...
    def get_domain_adaptation_config(self) -> TSMixerDomainAdaptationConfig:
        ...
        return TSMixerDomainAdaptationConfig(
            ...
            num_layers=self.NUM_LAYERS,
            dropout=self.DROPOUT,
            learning_rate=self.PRETRAIN_LR,  # Using full learning rate for adaptation
            group_identifier=self.GROUP_IDENTIFIER,
            lr_scheduler_patience=self.LR_SCHEDULER_PATIENCE,
            lr_scheduler_factor=self.LR_SCHEDULER_FACTOR,
